...
users.append(User(id=3,username='Naveen Kumar',password='naveen@123'))
...
def edit():
    ...
    if request.method == "POST":
        ...
        with sqlite3.connect("student_detials.db") as connection:
            cursor = connection.cursor() 
            qry="INSERT OR REPLACE INTO Student_Info VALUES(?,?,?,?,?,?,?)"
            cursor.execute(qry,(roll,name, email, gender, contact, dob, address)) 
            cursor.close()
...
def saveRecord():
    ...
    if request.method == "POST":
        try:
            ...
            with sqlite3.connect("student_detials.db") as connection:
                cursor = connection.cursor()
                qry="INSERT into Student_Info (roll,name, email, gender, contact, dob, address) values (?,?,?,?,?,?,?)"
                cursor.execute(qry,(roll,name, email, gender, contact, dob, address)) 
                connection.commit()
